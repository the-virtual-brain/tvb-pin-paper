...
\subsection{Node dynamics}
	...
	\note[sk]{I'd suggest focusing on different models. The implementation
	    of the mode decomposition, which is based on the papers, isn't 
	    functional as a generic decomposition -- it lacks a proper 
	    normalisation. Done properly you should be able to vary the number of 
	    modes and in doing so not qualitatively alter the dynamics but rather 
	    approach the ``true'' ensemble dynamics as you increase the number of 
	    modes, but this isn't possible as the equations stand, which in reality 
	    means that these models are actually just static high dimensional 
	    models -- that is, without an ability to modify the number of modes 
	    meaningfully, they could be equivalently recast as models with 3 times
...
\subsection{Network structure}
	...
	through a translation invarient
	...
	inhomogeneous parameters are supported -- thus being more general than
	the restricted case of translational invariance} 
	connectivity kernel, approximating a 
	neural field. 
